...
def spot_to_fieldsite(feat: dict) -> FieldSite:
    ...
    if images and (img := images[0] or {}) and img.get("id") is not None:
        photos.append(
            Photo(
                id=int(img["id"]),
                url="https://dev.rockd.org/api/v2/protected/photos",
...
#_________________IMAGE CONVERSION__________________________________________
...
async def _fetch_rockd_photo_bytes(person_id: int, photo_id: int, rockd_token: str) -> tuple[bytes, str]:
    async with httpx.AsyncClient(follow_redirects=True, timeout=60.0) as client:
        res = await client.get(
            f"{_get_rockd_api_url()}/protected/image/{person_id}/banner/{photo_id}",
            headers={"Accept": "*/*", "Authorization": f"Bearer {rockd_token}"},
        )
    if res.status_code >= 400:
        raise HTTPException(status_code=res.status_code, detail=f"Failed to fetch Rockd photo {photo_id} for person_id={person_id}: {res.text}")
