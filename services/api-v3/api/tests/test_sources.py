...
class TestAPI:
    ...
    def test_patch_source(self, api_client: TestClient):
        response = api_client.patch(
            f"/sources/{TEST_SOURCE_TABLE.source_id}", json={"url": "test"}
        )
    ...
    def test_post_source(self, api_client: TestClient):
        ...
        response = api_client.post("/sources", json=source)
    ...
    def test_get_sources_tables_default(self, api_client: TestClient):
        response = api_client.get(
            f"/sources/1/polygons?strat_name=group_by&page=0&page_size=999999"
        )
        assert response.status_code == 200
        response_json = response.json()
    ...
    def test_patch_source_tables_with_filter(self, api_client):
        ...
        response = api_client.patch(
            f"/sources/{TEST_SOURCE_TABLE.source_id}/polygons", json=body, params=params
        )

        assert response.status_code == 204

        response = api_client.get(
            f"/sources/{TEST_SOURCE_TABLE.source_id}/polygons", params=params
        )
    ...
    def test_patch_source_tables_with_filter_no_matches(self, api_client):
        ...
        assert (
            ...
            == "No rows patched, if this is unexpected please report as bug"
        )
    ...
    def test_group_by_source_table(self, api_client):
        ...
        for row in full_data:
            assert (
                str(row["_pkid"]) in comparison_values[row["PTYPE"]]
                or comparison_values[row["PTYPE"]] == "Multiple Values"
            )
    ...
    def test_order_by_source_table(self, api_client):
        ...
        assert all(
            [
                order_data[i]["_pkid"] <= order_data[i + 1]["_pkid"]
