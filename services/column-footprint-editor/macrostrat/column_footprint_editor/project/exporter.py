...
class ProjectExporter:
    ...
    def export_new_project(self):
        ...
        res = requests.post(
            self.base_url + "projects", headers=self.rep_header, data=data
        )
        res = res.json()
        project_id = res[0]["id"]
        ...
        for col_group in col_groups:
            data = {
                ...
                "col_group_long": col_group.col_group_name,
                "project_id": project_id,
            }
            res = requests.post(
                self.base_url + "col_groups", headers=self.rep_header, data=data
            )
            res = res.json()
        ...
        for col in cols:
            ...
            params = {
                "col_group_id": col_group_lookup[col.col_group],
                "project_id": project_id,
            }
            params = {
                **params,
