...
from psycopg2 import IntegrityError as DatabaseError

# TODO: Get geomaterial from DB as part of functions? Hack for now with a file
GEOMATERIALS = []
with open(
    os.path.join(os.path.split(os.path.abspath(__file__))[0], "geomats.txt"), "r"
) as file:
    for line in file:
...
GEMS_TABLE_FK_DICT = {
    ...
    "descriptionofmapunits": [
        ("descriptionsourceid", ("datasources", "datasources_id"))
    ],
...
GEMS_SPATIAL_TABLES_FIELDS = {
    "cartographiclines": ["cartographiclines_id", "shape", "ST_LineFromWKB"],
...
def replace_db_records(
    engine,
    db_metadata,
    target_table,
    field_to_match,
    value_to_match,
    target_field_value_dict,
):
    """
    ...
    stmt = (
        db.update(table_obj)
        .values(target_field_value_dict)
        .where(table_obj.c[field_to_match] == value_to_match)
    )
...
def test_ETL_tables_for_required_elements(engine, db_metadata, doPrint=True):
    ...
    for tbl in gemsDefDict:
        if not tbl in GEMS_TABLE_INGESTION_ORDER:
            ...
            missing_items["TABLE_INGESTION_ORDER"][tbl] = (
                "Table missing from ingestion order: {}".format(tbl)
            )
    ...
    for tbl in gemsShapeDict:
        if not tbl.lower() in GEMS_SPATIAL_TABLES_FIELDS:
            if not gemsShapeDict[tbl].lower() == "table":
                ...
                missing_items["SPATIAL_TABLES"][tbl] = (
                    "Table missing from spatial tables list: {}, needs spatial type: {}".format(
                        tbl.lower(), gemsShapeDict[tbl]
                    )
                )
    ...
    for tbl in gemsDefDict:
        if not tbl in GEMS_TABLE_FK_DICT:
            ...
            missing_items["TABLE_FK"][tbl] = (
                "Table missing from FK Dict order: {}".format(tbl)
            )
    ...
    for tbl in gemsDefDict:
        if tbl in GEMS_TABLE_FIELD_DICT:
            ...
            if fieldDiff:
                missing_items["TABLE_FIELD"][tbl] = (
                    "Fields missing for {} table: {}".format(tbl, fieldDiff)
                )
        else:
            missing_items["TABLE_FIELD"][tbl] = (
                "Table missing from ingestion order: {}".format(tbl)
    ...
    if doPrint:
        if missing_items:
            ...
            print(
                "Hooray, all required elements present in database! ヘ( ^o^)ノ＼(^_^ )"
            )
...
def test_engine_for_required_elements(engine, db_metadata, doPrint=True):
    ...
    for tbl in gemsDefDict:
        try:
            ...
            missing_items[tbl] = "Table missing, needs fields: {}".format(
                gemsDefDict[tbl]
            )
    ...
    if doPrint:
        if missing_items:
            print(
                "The following GeMS elements are missing from the target database: \n"
            )
...
def _transform_reset_geomaterial_values(table_df):
    ...
    table_df["descriptionofmapunits"]["geomaterial"] = table_df[
        "descriptionofmapunits"
    ]["geomaterial"].apply(
        lambda val: (
            None if val is None else difflib.get_close_matches(val, GEOMATERIALS)[0]
        )
    )
    return table_df
    ...
    for table in table_df:
        for col in table_df[table]:
            if col.lower() == "isconcealed":
                ...
                table_df[table][col] = table_df[table][col].str[
                    0
    ...
    for layer in gems_db_dict:
        ...
        if layer in GEMS_SPATIAL_TABLES_FIELDS:
            ...
            if ~np.all(table_df.is_valid):
                print(
                    "Invalid geometries found in table: {}, making valid...".format(
                        layer
                    )
                )
...
def _transform_drop_unused_gems_features(gems_db_dict, gems_features_to_ignore=None):
    ...
    for key in gems_db_dict:
        ...
        # If it isn't, get the fieldnames and save them as a list - but ignore standard gems features taht we aren't loading in because we want to standardize (e.g., glossary, geomaterialdict)
            ...
            unused_item_dict[key] = [
                field
                for field in gems_db_dict[key].columns
                if not (field in EXTRAITEMSTOIGNORE)
            ]

    return gems_db_dict_clean, unused_item_dict


def _transform_gems_concatenated_datasources_to_primarykeys(
    gems_db_dict, delimiter="|", new_delimiter="|"
):
    ...
    other_tables = [
        ...
        if (not (tbl == tablename) and (tbl in GEMS_TABLE_FK_DICT))
    ]
    ...
    for tbl in other_tables:
        ...
        for foreignkey, table_pk in fkfields_tables_pks:
            ...
            if table_pk[0] == tablename:
                ...
                delimitedIdcs = table_df[foreignkey].str.contains(
                ...
                if len(delimitedFields) > 0:
                    ...
                    for i in delimitedFields:
                        ...
                        key_components = np.sort(
                            table_df.loc[i, foreignkey].split(delimiter)
                        )  # Get the sorted delimiter entries
                        ...
                        new_concatdkey = new_delimiter.join(
                            [k.strip() for k in key_components]
                        )
    ...
    if concatd_ds:
        ...
        ds_toappend = pd.DataFrame(
            data=None, columns=ds_table.columns
        )  # Create a space to host the newly created datasource entries
        ...
        for i, new_ds in enumerate(concatd_ds):
            ...
            for col in ds_toappend.columns:
                if not (col == primarykey) and not (col == SELFREFERENCEFIELD):
                    ds_toappend.loc[i, col] = new_delimiter.join(
                        [str(val) for val in sub_df[col].values]
                    )

            # Track the list of primary keys that goes with each entry
            ds_toappend.loc[i, SELFREFERENCEFIELD].extend(
                [val for val in sub_df[primarykey].values]
            )

            # Prepend a little message that this was combined
            ds_toappend.loc[i, fieldToConcat] = (
                "Combination of multiple sources: " + ds_toappend.loc[i, fieldToConcat]
            )
...
def _transform_add_foreignkeyfields_to_accompany_mapunit(
    gems_db_dict, do_update_in_place=True
):
    """_summary_
...
def _transform_single_table_flatten_additional_columns_to_json(
    table_df: gpd.GeoDataFrame,
    columns_for_insert: list,
    do_update_in_place: bool = True,
    extra_attributes_column: str = None,
):
    ...
    columns_for_json = [
        ...
        if (not (c in columns_for_insert) and not (c in EXTRAITEMSTOIGNORE))
    ]
...
def _transform_flatten_additional_columns_to_json(gems_db_dict):
    ...
    for table in gems_db_dict:
        ...
        gems_db_dict[table] = (
            _transform_single_table_flatten_additional_columns_to_json(
                table_df, columns_to_preserve, extra_attributes_column=EXTRAITEMSFIELD
            )
        )
...
def transform_gems_dict_of_dataframes(
    gems_db_dict, mapsource_id, datasources_delimiter
):
    ...
    if not (datasources_delimiter is None):
        gems_db_dict = _transform_gems_concatenated_datasources_to_primarykeys(
            gems_db_dict, delimiter=datasources_delimiter, new_delimiter=DB_DELIMITER
        )
...
#### LOAD
###########################################################################################################################
def _load_update_foreignkeys_with_db_primarykeys(
    updated_pfkey_dict,
    target_table: str,
    table_df: gpd.GeoDataFrame,
    do_update_in_place=True,
    update_errors=None,
):
    """
        ...
        for fkfield, table_fkfield in fkfields_tables:
            ...
            for i, row in table_df.iterrows():
                ...
                try:
                    ...
                    cols_to_keep = [
                        col for col in row if col in GEMS_TABLE_FIELD_DICT[target_table]
                    ]
                    bad_row = row[cols_to_keep].to_dict()
                    bad_row["ErrorType"] = (
                        "Foreign key update error, no key {} found for field {}".format(
                            row[fkfield], fkfield
                        )
                    )
...
def _load_gems_table_update_keys(
    engine,
    db_metadata,
    updated_pfkey_dict,
    target_table: str,
    ...
    table_df, etl_errors = _load_update_foreignkeys_with_db_primarykeys(
        updated_pfkey_dict, target_table, table_df, update_errors=etl_errors
    )
    ...
    geomUpdateStr = """UPDATE {schema:}.{table:} SET {geom_field}=ST_Force2D({insert_func}(:wkb,{EPSG:})) WHERE {table_pk:}=:id;""".format(
    ...
    for i, row in table_df.iterrows():
        ...
        if SELFREFERENCEFIELD in columns_for_insert:
            if to_insert[SELFREFERENCEFIELD]:
                to_insert[SELFREFERENCEFIELD] = [
                    updated_pfkey_dict[(target_table, pk_field)][selfrefid]
                    for selfrefid in row[SELFREFERENCEFIELD]
                ]
        ...
        try:
            ...
            new_pkey = res.inserted_primary_key[
                0
            ]  # This appears to return a tuple of one item
            ...
            for pk_field in pk_fields:
                ...
                if (target_table, pk_field) in updated_pfkey_dict:
                    updated_pfkey_dict[(target_table, pk_field)][
                        row[pk_field]
                    ] = new_pkey
...
def _load_mapsource_poly(
    conn, mapsource_id, mapunitpolys_df, mapboundary_tolerance=100
):

    # Construct the sql statement that will be needed
    insert_func = "ST_GeomFromWKB"
    geomUpdateStr = """INSERT INTO source.mapsourcepolys (mapsourceid,shape) VALUES (:msid,ST_Force2D(ST_Multi({insert_func}(:wkb,{EPSG:}))));""".format(
        insert_func=insert_func, EPSG=PROJ_SRID
    )
    ...
    polygon = (
        mapunitpolys_df.geometry.unary_union
    )  # First combine all polygons into one shape
...
def _load_unused_tables_json(engine, db_metadata, mapsource_id, unused_item_dict):
    ...
    mapsources_primarykey = (
        db.inspect(
            get_table_object_from_gems_tablename(engine, db_metadata, MAPSOURCES_TBLNM)
        )
        .primary_key.columns.values()[0]
        .name
    )
    ...
    replace_db_records(
        ...
        mapsources_primarykey,
        mapsource_id,
        {EXTRAMAPSOURCEFEATURESFIELD: json_unused_items},
    )
...
def load_gems_database(
    ...
    for key in GEMS_TABLE_FK_DICT:
        ...
        for tk_i in table_key:
            ...
            if not (table_field_tuple in updated_pfkey_dict):
                updated_pfkey_dict[table_field_tuple] = (
                    {}
                )  # Create a place to dump foreign keys
    ...
    for tablename in GEMS_TABLE_INGESTION_ORDER:
        ...
        if tablename in gems_db_dict:
            ...
            updated_pfkey_dict, etl_errors_list = _load_gems_table_update_keys(
                engine,
                db_metadata,
                updated_pfkey_dict,
                tablename,
                table_df,
                mapsource_id,
                doAllowOnlySubsetOfGeMSFields=doAllowOnlySubsetOfGeMSFields,
            )
            ...
            if len(etl_errors_list) >= 1:
                etl_errors[tablename] = _etl_format_insertion_error_list(
                    etl_errors_list
                )
...
def etl_gems_database(
    ...
    try:
        ...
        # print('transform...')
        gems_db_dict, unused_item_dict = transform_gems_dict_of_dataframes(
            gems_db_dict, mapsource_id, datasources_delimiter
        )
        ...
        etl_errors["Overall error"] = {
            "Unexpected error, check GeMS Validation": pd.DataFrame(
                {"Error": [repr(e)]}
            )
        }
    ...
    if len(etl_errors) >= 1:
        print(
            "Errors detected in {} tables for mapsource: {}, removing associated data...".format(
                len(etl_errors), mapsource_id
            )
        )
    ...
    # If there were no insertion errors
        ...
        if "mapunitpolys" in gems_db_dict:
            ...
            _load_mapsource_poly(
                engine,
                mapsource_id,
                gems_db_dict["mapunitpolys"],
                mapboundary_tolerance=100,
            )
...
def etl_multiple_gdbs(
    engine,
    db_metadata,
    mapsources_etl_template,
    datasources_delimiter=None,
    doAllowOnlySubsetOfGeMSFields=False,
):
    ...
    if isinstance(mapsources_etl_template, str):
        ...
        if extension == ".csv":
            ...
            raise KeyError(
                "Error: ingestion template file must be either a .csv or .xlsx file"
            )
        ...
        with engine.connect().execution_options(isolation_level="AUTOCOMMIT") as conn:
            ...
            if os.path.exists(path_to_gdb):
                ...
                try:
                    ...
                    ingest_errors_i = etl_gems_database(
                        conn,
                        db_metadata,
                        path_to_gdb,
                        mapsource_id,
                        datasources_delimiter=datasources_delimiter,
                        doAllowOnlySubsetOfGeMSFields=doAllowOnlySubsetOfGeMSFields,
                    )
                ...
                print(
                    "No gdb found for source: {} at path {}".format(
                        row["source"], path_to_gdb
                    )
                )
...
if __name__ == "__main__":
    ...
    for name, frame in gems.items():
        fout = (
            OUT_DIR / f"{name}.gpkg"
            if name in GEMS_SPATIAL_TABLES_FIELDS
    ...
    if USE_DB:
        ...
        print(
            "  • loaded to Postgres;", "errors:" if errs else "no errors", errs.keys()
        )
    else:
