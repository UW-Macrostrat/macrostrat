...
def lookup_and_validate_age(name: str | float, interval_set: set[str]):
    ...
    for i, word in enumerate(name_array_of_strings):
        ...
        if word in {"early", "middle", "late", "upper", "lower"} and i + 1 < len(
            name_array_of_strings
        ):
            if word == "lower":
                word = "early"
            elif word == "upper":
                word = "late"
            if i + 3 <= len(name_array_of_strings) and name_array_of_strings[i + 2] in {
                "early",
                "middle",
                "late",
                "upper",
                ...
                phrase_two = (
                    f"{name_array_of_strings[i + 2]} {name_array_of_strings[i + 3]}"
                )
                if phrase_one in interval_set:
                    if phrase_two in interval_set:
                        # this is true if word is early or middle
                        if word == "early" and (
                            name_array_of_strings[i + 2] == "middle"
                            or name_array_of_strings[i + 2] == "late"
                        ):
...
def map_t_b_intervals(legend_df: G.DataFrame) -> G.DataFrame:
    ...
    legend_df["b_interval"] = legend_df["age"].apply(
        lookup_and_validate_age, args=(interval_set,)
    )
    needs_fill = legend_df["b_interval"].isna()
    legend_df.loc[needs_fill, "b_interval"] = legend_df.loc[needs_fill, "name"].apply(
        lookup_and_validate_age, args=(interval_set,)
    )
    return legend_df
