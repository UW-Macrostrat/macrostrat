...
def import_criticalmaas_map(filename: Path):
    ...
    fn = str(filename.absolute())
    print(fn)
    gpd = GeopackageDatabase(fn)
    ...
    polygon_type = gpd.get_dataframe("polygon_type", use_geopandas=True)
    geologic_unit = gpd.get_dataframe("geologic_unit", use_geopandas=True)
    ...
    df.to_postgis(
        polygon_table_name, engine.connect(), schema="sources", if_exists="append"
    )
...
# def main():
#     response = import_geopackage_map(
#         "/Users/clock/PycharmProjects/macrostrat-cli/cli/macrostrat/cli/io/criticalmaas/test/data/bc_kananaskis.gpkg"
#     )
