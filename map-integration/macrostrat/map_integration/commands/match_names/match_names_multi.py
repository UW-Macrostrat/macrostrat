...
class Processor(multiprocessing.Process):
    def __init__(self, task_queue, result_queue):
        ...
        self.pyConn = db.engine.raw_connection()
...
class Task(object):
    ...
    def __call__(self, connection=None):
        ...
        INSERT INTO maps.map_strat_names
            ...
            ) rocks ON ST_Intersects("""
                + (
                    "rocks.envelope"
                    if strictSpace
                    else "ST_Buffer(rocks.envelope, 1.2)"
                )
                + """, snft.geom)
            ...
            WHERE ((snft.best_t_age) < (intervals_bottom.age_bottom))
              ...
              AND rocks.%(field)s ~* concat('\y', """
                + (
                    "replace(snft.rank_name, '.', '')"
                    if strictNameMatch
                    else "replace(snft.name_no_lith, '.', '')"
                )
                + """, '\y')
            AND snft.strat_name_id IN (
              ...
              WHERE distinct_rocks.rock_match_field ~* concat('\y', """
                + (
                    "replace(lsn4.rank_name, '.', '')"
                    if strictNameMatch
                    else "replace(lsn4.name_no_lith, '.', '')"
                )
                + """, '\y')
              AND ST_Intersects(strat_name_footprints.geom, (
                SELECT """
                + ("rgeom " if strictSpace else "ST_Buffer(rgeom, 1.2) ")
                + """
                FROM maps.sources
                WHERE source_id = %(source_id)s
              ))
