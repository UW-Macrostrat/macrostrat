...
class Export(Base):
    ...
    def write_layer(
        self, filename, layer_name, geometry_type, select, select_params, table_schema
    ):
    ...
    def run(self):
        ...
        if len(self.args) == 2:
            ...
            Export.write_layer(
                self, filename, "units", "MultiPolygon", select, {}, Export.maps_schema
            )

            # Write homogenized lines
            select = Export.lines_select % (
                source_info["scale"],
                source_id,
            )
            Export.write_layer(
        ...
        # bbox mode
            ...
            print(
                "Invalid args. Please run macrostrat export --help to see available options"
            )
            sys.exit(1)
