...
class Seed(Base):
    ...
    def seed_mbtiles(self, layer):
        ...
        call(['tippecanoe -o %s/tiny.mbtiles --minimum-zoom=0 --maximum-zoom=2 --generate-ids --detect-shared-borders --simplification=5 -Lunits:<(ogr2ogr -f "GeoJSON" /dev/stdout "PG:host=%s port=%s dbname=%s user=%s" -sql "`cat %s`") -Llines:<(ogr2ogr -f "GeoJSON" /dev/stdout "PG:host=%s port=%s dbname=%s user=%s" -sql "`cat %s`")' % (mbtile_path, self.credentials['pg_host'], self.credentials['pg_port'], self.credentials['pg_db'], self.credentials['pg_user'], '%s/seed_scripts/sql/%s/tiny.sql' % (path, layer, ), self.credentials['pg_host'], self.credentials['pg_port'], self.credentials['pg_db'], self.credentials['pg_user'], '%s/seed_scripts/sql/%s/tiny_lines.sql' % (path, layer,), )], shell=True, stdout=FNULL, executable='/bin/bash')

        # Create tiles for small
        call(['tippecanoe -o %s/small.mbtiles --minimum-zoom=3 --maximum-zoom=5 --generate-ids --detect-shared-borders --simplification=6 -Lunits:<(ogr2ogr -f "GeoJSON" /dev/stdout "PG:host=%s port=%s dbname=%s user=%s" -sql "`cat %s`") -Llines:<(ogr2ogr -f "GeoJSON" /dev/stdout "PG:host=%s port=%s dbname=%s user=%s" -sql "`cat %s`")' % (mbtile_path, self.credentials['pg_host'], self.credentials['pg_port'], self.credentials['pg_db'], self.credentials['pg_user'], '%s/seed_scripts/sql/%s/small.sql' % (path, layer, ), self.credentials['pg_host'], self.credentials['pg_port'], self.credentials['pg_db'], self.credentials['pg_user'], '%s/seed_scripts/sql/%s/small_lines.sql' % (path, layer, ), )], shell=True, stdout=FNULL, executable='/bin/bash')

        # Create tiles for medium
        call(['tippecanoe -o %s/medium.mbtiles --minimum-zoom=6 --maximum-zoom=8 --generate-ids --detect-shared-borders --simplification=7 -Lunits:<(ogr2ogr -f "GeoJSON" /dev/stdout "PG:host=%s port=%s dbname=%s user=%s" -sql "`cat %s`") -Llines:<(ogr2ogr -f "GeoJSON" /dev/stdout "PG:host=%s port=%s dbname=%s user=%s" -sql "`cat %s`")' % (mbtile_path, self.credentials['pg_host'], self.credentials['pg_port'], self.credentials['pg_db'], self.credentials['pg_user'], '%s/seed_scripts/sql/%s/medium.sql' % (path, layer, ), self.credentials['pg_host'], self.credentials['pg_port'], self.credentials['pg_db'], self.credentials['pg_user'], '%s/seed_scripts/sql/%s/medium_lines.sql' % (path, layer, ), )], shell=True, stdout=FNULL, executable='/bin/bash')

        # Create tiles for large
        call(['tippecanoe -o %s/large.mbtiles --minimum-zoom=9 --maximum-zoom=12 --generate-ids --detect-shared-borders --simplification=4 -Lunits:<(ogr2ogr -f "GeoJSON" /dev/stdout "PG:host=%s port=%s dbname=%s user=%s" -sql "`cat %s`") -Llines:<(ogr2ogr -f "GeoJSON" /dev/stdout "PG:host=%s port=%s dbname=%s user=%s" -sql "`cat %s`")' % (mbtile_path, self.credentials['pg_host'], self.credentials['pg_port'], self.credentials['pg_db'], self.credentials['pg_user'], '%s/seed_scripts/sql/%s/large.sql' % (path, layer, ), self.credentials['pg_host'], self.credentials['pg_port'], self.credentials['pg_db'], self.credentials['pg_user'], '%s/seed_scripts/sql/%s/large_lines.sql' % (path, layer, ), )], shell=True, stdout=FNULL, executable='/bin/bash')
