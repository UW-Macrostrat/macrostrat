...
class Seed(Base):
    ...
    def seed_mbtiles(self, layer):
        ...
        call(['tippecanoe -o %s/medium.mbtiles --minimum-zoom=6 --maximum-zoom=8 --detect-shared-borders --simplification=4 -Lunits:<(ogr2ogr -f "GeoJSON" /dev/stdout "PG:host=%s port=%s dbname=%s user=%s" -sql "`cat %s`") -Llines:<(ogr2ogr -f "GeoJSON" /dev/stdout "PG:host=%s port=%s dbname=%s user=%s" -sql "`cat %s`")' % (mbtile_path, self.credentials['pg_host'], self.credentials['pg_port'], self.credentials['pg_db'], self.credentials['pg_user'], '%s/seed_scripts/sql/%s/medium.sql' % (path, layer, ), self.credentials['pg_host'], self.credentials['pg_port'], self.credentials['pg_db'], self.credentials['pg_user'], '%s/seed_scripts/sql/%s/medium_lines.sql' % (path, layer, ), )], shell=True, stdout=FNULL, executable='/bin/bash')
