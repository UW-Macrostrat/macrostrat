...
class Backup(Base):
    ...
    def run(self):
        ...
        if self.args[1].isdigit():
            ...
            call(
                [
                    'echo "COPY maps.sources FROM stdin;" | cat - temp_sources.tsv > sources.tsv && echo "\.\n" >> sources.tsv && rm -f temp_sources.tsv'
                ],
                shell=True,
            )
            ...
            call(
                [
                    'echo "COPY maps.%s FROM stdin;" | cat - temp_scale.tsv > scale.tsv && echo "\.\n" >> scale.tsv && rm -f temp_scale.tsv'
                    % (source_info.scale,)
                ],
                shell=True,
            )
            ...
            call(
                [
                    'echo "COPY lines.%s FROM stdin;" | cat - temp_lines.tsv > lines.tsv && echo "\.\n" >> lines.tsv && rm -f temp_lines.tsv'
                    % (source_info.scale,)
                ],
                shell=True,
            )
            ...
            call(
                [
                    'echo "COPY maps.map_liths FROM stdin;" | cat - temp_map_liths.tsv > map_liths.tsv && echo "\.\n" >> map_liths.tsv && rm -f temp_map_liths.tsv'
                ],
                shell=True,
            )
            ...
            call(
                [
                    'echo "COPY maps.map_units FROM stdin;" | cat - temp_map_units.tsv > map_units.tsv && echo "\.\n" >> map_units.tsv && rm -f temp_map_units.tsv'
                ],
                shell=True,
            )
            ...
            call(
                [
                    'echo "COPY maps.legend FROM stdin;" | cat - temp_legend.tsv > legend.tsv && echo "\.\n" >> legend.tsv && rm -f temp_legend.tsv'
                ],
                shell=True,
            )
            ...
            call(
                [
                    'echo "COPY maps.map_legend FROM stdin;" | cat - temp_map_legend.tsv > map_legend.tsv && echo "\.\n" >> map_legend.tsv && rm -f temp_map_legend.tsv'
                ],
                shell=True,
            )
            ...
            call(
                [
                    'echo "COPY maps.legend_liths FROM stdin;" | cat - temp_legend_liths.tsv > legend_liths.tsv && echo "\.\n" >> legend_liths.tsv && rm -f temp_legend_liths.tsv'
                ],
                shell=True,
            )
            ...
            call(
                [
                    'echo "COPY points.points FROM stdin;" | cat - temp_points.tsv > points.tsv && echo "\.\n" >> points.tsv && rm -f temp_points.tsv'
                ],
                shell=True,
            )
            ...
            for scale in scaleIsIn[source_info.scale]:
                ...
                call(
                    [
                        'echo "COPY carto_new.%s FROM stdin;" | cat - temp_carto_%s.tsv > carto_%s.tsv && echo "\.\n" >> carto_%s.tsv && rm -f temp_carto_%s.tsv'
                ...
                call(
                    [
                        'echo "COPY carto_new.lines_%s FROM stdin;" | cat - temp_carto_lines_%s.tsv > carto_lines_%s.tsv && echo "\.\n" >> carto_lines_%s.tsv && rm -f temp_carto_lines_%s.tsv'
                        % (scale, scale, scale, scale, scale)
                    ],
                    shell=True,
                )
            ...
            call(
                "cat <(cat %s/export_scripts/delete.sql | sed 's/::source_id::/%s/g; s/::scale::/%s/g; s/::primary_table::/%s/g; s/::primary_line_table::/%s/g') <(pg_dump -O -x -c -t sources.%s -t sources.%s -U %s -h %s -p %s burwell | cat - sources.tsv scale.tsv lines.tsv map_liths.tsv strat_names.tsv map_units.tsv points.tsv lookup.tsv) <(%s) <(cat %s) | gzip > %s.%s.sql.gz && rm -f *.tsv"
                % (
                    os.path.dirname(__file__),
                    self.args[1],
                    source_info.scale,
                    source_info.primary_table,
                    source_info.primary_line_table,
                    source_info.primary_table,
                    source_info.primary_line_table,
            ...
            print(
                "     Dumped %s to %s.%s.sql.gz"
                % (source_info.name, today, source_info.name.replace(" ", "_"))
            )
            ...
            if db in pg_dbs:
                if "db" == "burwell":
                    call(
                        [
                            "pg_dump -O -x -T sources.etopo1 -U %s -h %s -p %s burwell | gzip > %s.burwell.sql.gz"
                            % (
                                self.credentials["pg_user"],
                                self.credentials["pg_host"],
                                self.credentials["pg_port"],
                    ...
                    call(
                        [
                            "pg_dump -O -x -U %s -h %s -p %s %s | gzip > %s.%s.sql.gz"
                            % (
                                self.credentials["pg_user"],
                                self.credentials["pg_host"],
                                self.credentials["pg_port"],
                ...
                call(
                    [
                        "mysqldump -u %s --password=%s -h %s -S %s %s | gzip > %s.%s.sql.gz"
                        % (
                            self.credentials["mysql_user"],
                            self.credentials["mysql_passwd"],
                            self.credentials["mysql_host"],
                            self.credentials["mysql_socket"],
                            db,
                            today,
                            db,
                        )
