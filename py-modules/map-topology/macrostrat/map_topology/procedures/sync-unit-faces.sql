/** A face per compilation, alongside the faces of its constituents.
...
FROM (
  ...
  JOIN map_bounds_topology.map_face f
   ...
   AND f.map_id = mp.source_id
  JOIN map_bounds_topology.relation r
    ON r.layer_id = (f.topo).layer_id
   AND r.topogeo_id = (f.topo).id
   AND r.element_type = 3
  -- Only where a compilation is involved. A map that stands alone is its own
  -- unit, and its existing faces already are the answer.
  WHERE mp.via IS DISTINCT FROM mp.source_id
  GROUP BY mp.map_layer, mp.via
) u
CROSS JOIN LATERAL (
