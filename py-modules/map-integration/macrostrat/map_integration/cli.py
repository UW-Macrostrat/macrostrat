...
def create_tables(
    slug: str = Argument(..., help="Slug for the source"),
    kind: list[str] = Option(
        None,
        "--kind",
        "-k",
        help="Table kinds to create; repeatable. Defaults to all three.",
    ),
    srid: int = Option(4326, help="SRID for the geometry columns"),
):
    """Create empty `sources.*` staging tables at the current standard.

    For data that does not arrive as a GIS file, where `prepare-fields` has no
    table to bring up to standard. Source-specific columns are not expressible
    here -- pass `extra_columns` to `create_source_tables` from an ingestion
    script for those.
    """
    from .source_tables import COLUMN_SPECS, create_source_tables
