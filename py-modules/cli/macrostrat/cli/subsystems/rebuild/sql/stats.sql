...
UPDATE stats_new SET burwell_polygons = (SELECT SUM(features) FROM maps.sources WHERE status_code='active');

ALTER TABLE IF EXISTS stats rename to stats_old;
ALTER TABLE IF EXISTS stats_new rename to stats;
