...
def get_data_counts_pg():
    ...
    with engine.connect() as conn:
        table_query = text(f"SELECT table_name FROM information_schema.tables WHERE table_catalog = '{pg_db_name_two}'" \
                     " AND table_type = 'BASE TABLE'" \
                     f" AND table_schema = 'macrostrat_temp'")
        table_result = conn.execute(table_query)
        pg_tables = [row[0] for row in table_result]
        for table in pg_tables:
            row_query = text(f"SELECT COUNT(*) FROM {pg_db_name_two}.macrostrat_temp.{table};")
            row_result = conn.execute(row_query)
            row_count = row_result.scalar()
            pg_rows[table.lower()] = row_count

            column_query = text(f"SELECT COUNT(*) FROM information_schema.columns WHERE table_catalog = '{pg_db_name_two}' AND table_schema = 'macrostrat_temp' AND table_name = '{table}';")
            column_result = conn.execute(column_query)
            column_count = column_result.scalar()
...
"""
Compares the data counts between tables, rows, and columns that vary between MariaDB and PostgreSQL
"""
def compare_data_counts(maria_rows, pg_rows, maria_columns, pg_columns):
    maria_rows_not_in_pg = {table_name: (maria_rows[table_name], 0) for table_name in maria_rows if
                            table_name not in pg_rows}
    pg_rows_not_in_maria = {table_name: (0, pg_rows[table_name]) for table_name in pg_rows if
                            table_name not in maria_rows}
    maria_cols_not_in_pg = {table_name: (maria_columns[table_name], 0) for table_name in maria_columns if
                            table_name not in pg_columns}
    pg_cols_not_in_maria = {table_name: (0, pg_columns[table_name]) for table_name in pg_columns if
                            table_name not in maria_columns}

    if len(maria_rows_not_in_pg) == 0 and len(pg_rows_not_in_maria) == 0:
        print("Success! All tables exist in both MariaDB and PostgreSQL. Checking row counts....\n")
    else:
        if len(maria_rows_not_in_pg) > 0:
            print('ERROR: MariaDB tables that are not in PostgreSQL:\n', [key for key in maria_rows_not_in_pg])
        if len(pg_rows_not_in_maria) > 0:
            print('ERROR: PostgreSQL tables that are not in MariaDB: \n', [key for key in pg_rows_not_in_maria])

    row_count_difference = {key: (maria_rows[key], pg_rows[key]) for key in maria_rows if
                            key in pg_rows and maria_rows[key] != pg_rows[key]}
    row_count_difference.update(maria_rows_not_in_pg)
    row_count_difference.update(pg_rows_not_in_maria)

    col_count_difference = {key: (maria_columns[key], pg_columns[key]) for key in maria_columns if
                            key in pg_columns and maria_columns[key] != pg_columns[key]}
    col_count_difference.update(maria_cols_not_in_pg)
    col_count_difference.update(pg_cols_not_in_maria)

    if len(row_count_difference) == 0:
        print("Success! All row counts in all tables are the same in both MariaDB and PostgreSQL!\n")
    else:
        print(f'ERROR: Row count differences for {len(row_count_difference)} tables in both MariaDB and PostgreSQL databases:\n'
              '{Table Name: (Maria Rows, PG Rows)}\n'
              f'{row_count_difference}')
    if len(col_count_difference) == 0:
        print("Success! All column counts in all tables are the same in both MariaDB and PostgreSQL!\n")
    else:
        print(f'ERROR: Column count differences for {len(col_count_difference)} tables in both MariaDB and PostgreSQL databases:\n'
              '{Table Name: (Maria Columns, PG Columns)}\n'
...
if __name__ == "__main__":
    ...
    row_variance, column_variance = compare_data_counts(maria_rows, pg_rows, maria_columns, pg_columns)
    #reset()