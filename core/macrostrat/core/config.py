...
def list_of_resolved_paths(value, *, require_directory=False, require_file=False):
    ...
    for p in _paths:
        ...
        if require_file and not p1.is_file():
            raise ValueError(f"{p1} is not a {element_type}")
        _out.append(p1.resolve())
    return _out


def list_of_directories(value):
    return list_of_resolved_paths(value, require_directory=True)


def list_of_files(value):
    return list_of_resolved_paths(value, require_file=True)


settings.validators.register(
    # `must_exist` is causing huge problems
    Validator("COMPOSE_ROOT", cast=Path),
    Validator("env_files", cast=list_of_files, default=None),
    Validator("script_dirs", cast=list_of_directories, default=None),
